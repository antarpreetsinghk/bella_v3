...
from app.services.simple_extraction import extract_appointment_fields
    ...
    *,
    ...
    try:
        calendar_event = await create_calendar_event(
            ...
            starts_at_utc=appt.starts_at.astimezone(UTC),
        ...
        if calendar_event:
            ...
            try:
                ...
                await db.execute(
                    ...
                    .values(google_event_id=calendar_event.get('event_id'))
                )
                await db.commit()
                logger.info("Updated appointment %s with google_event_id: %s", appt.id, calendar_event.get('event_id'))
            except Exception as update_e:
                logger.warning("Failed to update appointment %s with google_event_id: %s", appt.id, update_e)

...
echo "🐳 Building Lambda dependencies with Docker (using sudo)..."

# Build the Docker image
sudo docker build -f Dockerfile.lambda-deps -t lambda-deps-builder .

# Create a container and extract packages
echo "📦 Extracting built packages..."

# Create temporary container
CONTAINER_ID=$(sudo docker create lambda-deps-builder)
...
sudo docker cp ${CONTAINER_ID}:/lambda-packages/. lambda-deployment-minimal/

# Fix permissions after copying from Docker
sudo chown -R $(whoami):$(whoami) lambda-deployment-minimal/
